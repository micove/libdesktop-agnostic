/* vfs.c generated by valac, the Vala compiler
...
GType desktop_agnostic_vfs_implementation_get_type (void) {
	static GType desktop_agnostic_vfs_implementation_type_id = 0;
	if (desktop_agnostic_vfs_implementation_type_id == 0) {
	...
	return desktop_agnostic_vfs_implementation_type_id;
}


DesktopAgnosticVFSImplementation* desktop_agnostic_vfs_get_default (GError** error) {
	DesktopAgnosticVFSImplementation* result;
	...
	if (desktop_agnostic_vfs_vfs == NULL) {
		...
		if (type == G_TYPE_INVALID) {
			...
			desktop_agnostic_vfs_vfs = (_tmp0_ = DESKTOP_AGNOSTIC_VFS_IMPLEMENTATION (g_object_new (type, NULL)), _g_object_unref0 (desktop_agnostic_vfs_vfs), _tmp0_);
		}
	}
