/* vfs-file-impl-gnome-vfs.c generated by valac, the Vala compiler
...
static gboolean desktop_agnostic_vfs_file_gnome_vfs_handle_error (DesktopAgnosticVFSFileGnomeVFS* self, GnomeVFSResult res, GError** error) {
	...
	if (res == GNOME_VFS_OK) {
		...
		GError* err;
		err = (GError*) g_error_new (g_quark_from_string ("Gnome.VFS"), 0, gnome_vfs_result_to_string (res), NULL);
...
static gboolean desktop_agnostic_vfs_file_gnome_vfs_real_load_contents (DesktopAgnosticVFSFile* base, char** contents, gsize* length, GError** error) {
	...
	buffer = (_tmp3_ = g_new0 (gchar, (gint) info->size), buffer = (g_free (buffer), NULL), buffer_length1 = (gint) info->size, buffer_size = buffer_length1, _tmp3_);
...
GType desktop_agnostic_vfs_file_gnome_vfs_get_type (void) {
	static GType desktop_agnostic_vfs_file_gnome_vfs_type_id = 0;
	if (desktop_agnostic_vfs_file_gnome_vfs_type_id == 0) {
	...
	return desktop_agnostic_vfs_file_gnome_vfs_type_id;
}
