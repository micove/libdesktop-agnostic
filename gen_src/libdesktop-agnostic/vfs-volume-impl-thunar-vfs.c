/* vfs-volume-impl-thunar-vfs.c generated by valac, the Vala compiler
...
DesktopAgnosticVFSVolumeThunarVFS* desktop_agnostic_vfs_volume_thunar_vfs_construct_for_implementation (GType object_type, ThunarVfsVolume* impl) {
	...
	__params_it->name = "implementation";
	g_value_init (&__params_it->value, THUNAR_VFS_TYPE_VOLUME);
	g_value_set_object (&__params_it->value, impl);
	__params_it++;
...
gboolean desktop_agnostic_vfs_volume_thunar_vfs_do_mount (DesktopAgnosticVFSVolumeThunarVFS* self) {
	...
	self->priv->_mount_callback = (_tmp0_ = NULL, ((self->priv->_mount_callback_target_destroy_notify == NULL) ? NULL : self->priv->_mount_callback_target_destroy_notify (self->priv->_mount_callback_target), self->priv->_mount_callback = NULL, self->priv->_mount_callback_target = NULL, self->priv->_mount_callback_target_destroy_notify = NULL), self->priv->_mount_callback_target = NULL, self->priv->_mount_callback_target_destroy_notify = NULL, _tmp0_);
	result = FALSE;
	return result;
}
...
static gboolean _desktop_agnostic_vfs_volume_thunar_vfs_do_mount_gsource_func (gpointer self) {
	return desktop_agnostic_vfs_volume_thunar_vfs_do_mount (self);
}
...
static gboolean _desktop_agnostic_vfs_volume_thunar_vfs_do_unmount_gsource_func (gpointer self) {
	return desktop_agnostic_vfs_volume_thunar_vfs_do_unmount (self);
}
...
static gboolean _desktop_agnostic_vfs_volume_thunar_vfs_do_eject_gsource_func (gpointer self) {
	return desktop_agnostic_vfs_volume_thunar_vfs_do_eject (self);
}
...
GType desktop_agnostic_vfs_volume_thunar_vfs_get_type (void) {
	static GType desktop_agnostic_vfs_volume_thunar_vfs_type_id = 0;
	if (desktop_agnostic_vfs_volume_thunar_vfs_type_id == 0) {
	...
	return desktop_agnostic_vfs_volume_thunar_vfs_type_id;
}
...
GType desktop_agnostic_vfs_volume_monitor_thunar_vfs_get_type (void) {
	static GType desktop_agnostic_vfs_volume_monitor_thunar_vfs_type_id = 0;
	if (desktop_agnostic_vfs_volume_monitor_thunar_vfs_type_id == 0) {
	...
	return desktop_agnostic_vfs_volume_monitor_thunar_vfs_type_id;
}
